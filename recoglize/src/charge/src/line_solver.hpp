...
public:
    ...
    static double distToLine(LinePara& para,const Point& point){
    ...
    double solve(const std::vector<Point> &points, LinePara &para) {
        if (points.size() <= 4) {
        ...
        // double theta = atan2( sum_uv, sum_uu);
    ...
    static bool solveByToPoint(Point &start,Point &end, LinePara &para) {
...
private:
    ...
    const double abandon_ration = 0.0625;//根据L型的长宽确定
};
