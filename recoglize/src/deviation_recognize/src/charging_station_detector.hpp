...
struct ChargingStationDetectInfo {  //站点识别信息
    double direction = 0;
    Point center_point;     //中心点
    Point first_corner;     //第一个交点
    Point second_corner;    //第二个交点
    Point first_top;    //第一个腰点，右侧
    Point second_top;   //第二个腰点，左侧
};
...
public:
    ...
    bool findChargingStation(ScanType &scan, 
        ChargingStationInfo &info,std::vector<detector::ChargingStationDetectInfo> &detect_infos) {
    ...
    bool findChargingStation(ScanType &scan, std::vector<ContinousPoints> &continous_points,
        ...
        auto &check_length = info.baseline; //0.496m
        ...
        for (auto &segment:continous_points) {
            ...
            //*************寻找腰点*******************
            if (inRange((first_corner.point - second_corner.point).norm(), info.baseline, 2 * max_dist_err)) {//两侧角点判断
                ...
                for (int j = first_corner.index; j < second_corner.index; ++j) {
                    ...
                    double delta_err_1 = std::hypot(delta_err_1_1, delta_err_1_2);  //计算模长
...
//                    return true;
    ...
    //已知点A、B间的任一点C，计算向量ABxAC的的绝对值最大值
    ...
    //计算point到第一个点first和第二个点second的距离dist_1，dist_2
    ...
    void computeChargingCenterPose(const ScanType &scan, const Corner &first_corner, const Corner &first_top,
        ...
        Point line_origin_point;    //直线上line_para_1的一个点
...
private:
    const double max_laser_range = 3.0; //最大激光识别距离,单位m
    const double min_laser_range = 0.3; //最小激光识别距离,单位m
