...
class ModbusBaseRequestHandler(socketserver.BaseRequestHandler):
    ...
    def execute(self, request):
        ...
        try:
            ...
            #@mark time out, there catch ModbusIOException and just return, donot send response to client
            _logger.debug("ModbusIOException: %s", ex)
            return
        ...
        if not broadcast:
            ...
            #_logger.debug("request.function_code: %s" % request.function_code)
            #_logger.debug("response.function_code: %s" % response.function_code)

            #正常应答时,如果功能码不一致，则不发送
            if response.isError() == False and response.function_code != request.function_code:
                return
                #response = request.doException(merror.SlaveFailure)
