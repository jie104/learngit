...
InitScanGrid(DmtxDecode *dec)
{
   ...
   scale = dmtxDecodeGetProp(dec, DmtxPropScale);//scale = 1;
   smallestFeature = dmtxDecodeGetProp(dec, DmtxPropScanGap) / scale;//扫描间隔,scan_gap = 1;
   ...
   grid.yMax = dmtxDecodeGetProp(dec, DmtxPropYmax);//设置图像断点像素

   /* Values that get set once */
   xExtent = grid.xMax - grid.xMin;
   yExtent = grid.yMax - grid.yMin;
   maxExtent = (xExtent > yExtent) ? xExtent : yExtent;//获取长边
