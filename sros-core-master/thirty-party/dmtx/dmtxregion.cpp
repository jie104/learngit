...
dmtxRegionFindNext(DmtxDecode *dec, DmtxTime *timeout)
{
   ...
   for(;;) {
//      printf("the loc :%d,%d,%d,%d,%d\n", loc.X, loc.Y,dec->grid.xCenter,dec->grid.yCenter,dec->grid.pixelCount);
...
dmtxRegionScanPixel(DmtxDecode *dec, int x, int y)
{
   ...
   flowBegin = MatrixRegionSeekEdge(dec, loc);//找到可组成直线的三个点
...
MatrixRegionSeekEdge(DmtxDecode *dec, DmtxPixelLoc loc)
{
   ...
   flowPos = FindStrongestNeighbor(dec, flow, +1);//自y方向搜索,x方向用于设置当前flow
   ...
   }//这里的条件判断是说,如果三个点形成了一条直线,那么认为当前flow比较靠谱
...
MatrixRegionOrientation(DmtxDecode *dec, DmtxRegion *reg, DmtxPointFlow begin)
{
   ...
   err = TrailBlazeContinuous(dec, reg, begin, maxDiagonal);//找到轮廓,记录了边界点
   ...
   line1x = FindBestSolidLine(dec, reg, 0, 0, +1, DmtxUndefined);//通过hough变换检测直线
   if(line1x.mag < 5) {
      TrailClear(dec, reg, 0x40);
      return DmtxFail;
   }
   ...
   fTmp = FollowSeek(dec, reg, line1x.stepPos + 5);//找到下一个起点,然后找第二条直线,这里line1x.stepPos + 5表示当前位置的偏移量
   line2p = FindBestSolidLine(dec, reg, fTmp.step, line1x.stepNeg, +1, line1x.angle);

   fTmp = FollowSeek(dec, reg, line1x.stepNeg - 5);//从两个端点开始找,找到的那个,为真正的另外一条直线
   line2n = FindBestSolidLine(dec, reg, fTmp.step, line1x.stepPos, -1, line1x.angle);
   if(max(line2p.mag, line2n.mag) < 5)
      return DmtxFail;

   if(line2p.mag > line2n.mag) {//判断方向,然后确定赋值
      line2x = line2p;
      ...
      cross = ((line1x.locPos.X - line1x.locNeg.X) * (line2x.locPos.Y - line2x.locNeg.Y)) -
            ((line1x.locPos.Y - line1x.locNeg.Y) * (line2x.locPos.X - line2x.locNeg.X));//叉乘表示是逆时针还是顺时针,这里
      // 叉乘的定义方向指向角点,因此,刚好是相反的,所以,一般情况下是选择condition3
      if(cross > 0) {
         /* Condition 2 */
         reg->polarity = +1;
         ...
         /* Condition 3 */
         reg->polarity = -1;//表示方向,指向二维码原点的方向
...
dmtxRegionUpdateXfrms(DmtxDecode *dec, DmtxRegion *reg)
{
   ...
   if(dmtxRay2Intersect(&p01, &rTop, &rLeft) == DmtxFail)//求两个直线的交点
      return DmtxFail;
 ...
 */
 DmtxPointFlow
GetPointFlow(DmtxDecode *dec, int colorPlane, DmtxPixelLoc loc, int arrive)//判断增长最大方向
{
   ...
   for(patternIdx = 0; patternIdx < 8; patternIdx++) {
      xAdjust = loc.X + dmtxPatternX[patternIdx];//8个方向搜索
      yAdjust = loc.Y + dmtxPatternY[patternIdx];
      err = dmtxDecodeGetPixelValue(dec, xAdjust, yAdjust, colorPlane,
            &colorPattern[patternIdx]);//将8个方向搜索到的像素信息,保存在colorpattern中
      if(err == DmtxFail)
         return dmtxBlankEdge;
   }
   ...
   for(compass = 0; compass < 4; compass++) {
      ...
      /* Add portion from each position in the convolution matrix pattern *///卷积矩阵
      for(patternIdx = 0; patternIdx < 8; patternIdx++) {
         ...
         switch(coefficient[coefficientIdx]) {//一共四个方向,首先是左前,然后竖直,然后右前,最后水平;右侧为正值,左侧为负值
            case 2:
               mag[compass] += color;//注意,这里距离中心最远的角点,其进行了两次叠加
               /* Fall through */
            case 1:
               mag[compass] += color;
               break;
   ...
   flow.plane = colorPlane;//记录当前channel
   flow.arrive = arrive;
   flow.depart = (mag[compassMax] > 0) ? compassMax + 4 : compassMax;//记录符号?
   flow.mag = abs(mag[compassMax]);//记录最大值
   flow.loc = loc;
...
FindStrongestNeighbor(DmtxDecode *dec, DmtxPointFlow center, int sign)
{
   ...
   for(i = 0; i < 8; i++) {
      ...
      if((int)(*cache & 0x80) != 0x00) {//如果当前点已经搜索过,则返回
         if(++occupied > 2)
            return dmtxBlankEdge;
         else
            continue;
...
/**
 *对当前输入的loc,获取种子信息,链码信息,信息存储在neighbor中,ptr为当前指针
 *
 */
 DmtxFollow
...
FollowStep(DmtxDecode *dec, DmtxRegion *reg, DmtxFollow followBeg, int sign)
{
   ...
   /* Trail in progress -- normal jump */
      ...
      patternIdx = (sign < 0) ? followBeg.neighbor & 0x07 : ((followBeg.neighbor & 0x38) >> 3);//存储为链码,直接通过该方式即可获得下一个轮廓位置
...
TrailBlazeContinuous(DmtxDecode *dec, DmtxRegion *reg, DmtxPointFlow flowBegin, int maxDiagonal)
{
   ...
   cacheBeg = dmtxDecodeGetCache(dec, flowBegin.loc.X, flowBegin.loc.Y);//begin cache
   if(cacheBeg == NULL)
      return DmtxFail;
   *cacheBeg = (0x80 | 0x40); /* Mark location as visited and assigned *///标记之后,不会再次寻找

   reg->flowBegin = flowBegin;

   posAssigns = negAssigns = 0;
   for(sign = 1; sign >= -1; sign -= 2) {//寻找两个方向,先往前寻找
      ...
      for(steps = 0; ; steps++) {
         ...
         *cacheNext |= (0x80 | 0x40); /* Mark location as visited and assigned *///做标记
         if(sign > 0)
            posAssigns++;
         else
            negAssigns++;
         ...
         if (sign > 0) {
            reg->points.push_back(flow.loc);
         }else {
            reg->neg_points.push_back(flow.loc);
         }
 ...
 * ratcheting bresline inward or backward (although back + outward is allowed).//防止锯齿影响
 *
 */
 int
...
FindBestSolidLine(DmtxDecode *dec, DmtxRegion *reg, int step0, int step1, int streamDir, int houghAvoid)
{
   ...
   for(step = 0; step < tripSteps; step++) {
      ...
      for(i = 0; i < DMTX_HOUGH_RES; i++) {
         ...
         dH = (rHvX[i] * yDiff) - (rHvY[i] * xDiff);//x = cos(angle)*256,y=sin(angle)*256,angle取值为0-180°,步长为1°;根据
         // cos(theta)*(y-y0)-sin(theta)*(x-x0) = 0,得到方程,然后求得anglebest,该出求最小值即是
...
FindTravelLimits(DmtxDecode *dec, DmtxRegion *reg, DmtxBestLine *line)
{
   ...
   for(i = 0; i < reg->stepsTotal/2; i++) {//向前向后均有搜索,因此,除以2

      posRunning = (int)(i < 10 || abs(posWander) < abs(posTravel));//poswander如果大于posetravel,则该点必然不在该直线上
      ...
      if(posRunning != 0) {
         ...
         if(posWander >= -3*256 && posWander <= 3*256) {//posWander,表示的是高,因为,两者向量叉乘,其中一个向量模为1时,另一个向量的高即是叉乘结果
            distSq = DistanceSquared(followPos.loc, negMax);
            if(distSq > distSqMax) {
               posMax = followPos.loc;
               distSqMax = distSq;
               line->stepPos = followPos.step;//step表示距离当前位置的远近
...
MatrixRegionAlignCalibEdge(DmtxDecode *dec, DmtxRegion *reg, int edgeLoc)
{
   ...
   locOrigin.Y = (int)(pTmp.Y + 0.5);//求当前位置
   ...
   if(edgeLoc == DmtxEdgeTop) {
      ...
      dmtxMatrix3VMultiplyBy(&pTmp, reg->fit2raw);//是一个世界坐标系到图像坐标系的坐标变换,其中,世界坐标系的边界为(1,1)
      pTmp.X = 0.8;
   ...
   loc0 = follow.loc;
   line = BresLineInit(loc0, loc1, locOrigin);//制作bresenham直线
   ...
   if(edgeLoc == DmtxEdgeTop) {
      reg->topKnown = 1;
      reg->topAngle = bestLine.angle;//只改变了角度
