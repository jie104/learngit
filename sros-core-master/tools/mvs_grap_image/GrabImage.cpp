...
void convertToCvMat(void* handle, const MV_FRAME_OUT &stOutFrame){
    
    int img_buf_size = stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight * 3 + 2048;
    unsigned char *m_pBufForSaveImage = (unsigned char*)malloc(img_buf_size);
    memset(m_pBufForSaveImage,0,img_buf_size);
    ...
    if (isMono)
    {
        ...
        stConvertParam.nDstBufferSize = img_buf_size;                    
        int nRet = MV_CC_ConvertPixelType(handle ,&stConvertParam);

        frame = cv::Mat(stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nWidth, CV_8UC3, m_pBufForSaveImage);
...
static  void* WorkThread(void* pUser)
{
    ...
    while(1)
    {
        ...
        if (nRet == MV_OK)
        {
            convertToCvMat(pUser, stOutFrame);
