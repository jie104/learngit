...
#ifndef SROS_SH100_SENSOR_MANAGER_HPP
#define SROS_SH100_SENSOR_MANAGER_HPP

#include <Eigen/Dense>
#include "../rack_oba_filter_singleton.h"
#include "base_sensor_manager.hpp"
#include "core/device/can_interface.h"
#include "core/hardware/SH100.h"
#include "functional"

namespace sensor {
class Sh100SensorManager : public BaseSensorManager {
 public:
    Sh100SensorManager(oba::ObstacleModulePara_Ptr para, ObaMsgCallback msg)
        : BaseSensorManager(para, msg, TYPE_SH100_SENSOR) {
        oba_name_ = "SH100_OBA";

        sh100_tfs_.resize(para->tof_nums);//需在设备初始化前完成赋值，否则容易死机。
        tof_devices_.resize(para->tof_nums);
        for (uint32_t i = 0; i < para->tof_nums; ++i) {
            sh100_tfs_[i] = Eigen::Affine2d(Eigen::Translation2d(para->tof_coords[i][0],para->tof_coords[i][1]) *
                                            Eigen::Rotation2Dd(para->tof_coords[i][2]));
            LOG(INFO) << "tof" << i + 1 << "coord info:" << para->tof_coords[i][0] << "," << para->tof_coords[i][1]
                      << "," << para->tof_coords[i][2];
        }
        LOG(INFO) << "successfully to initialize sh100!";

        sh100_ptr_agv_.resize(para->sh100_nums);
        int sh100_device_id = 631;
        const int each_sh100_tof_num = para->each_sh100_tof_num;
        for (uint32_t j = 0; j < para->sh100_nums; ++j) {
            if (para->sh100_infos[j].sh100_enable_state) {
                int sh100_id = j + 1;
                std::string device_name = "SH100_TOF_" + std::to_string(sh100_id);
                sros::device::DeviceID device_id = (sros::device::DeviceID)(sh100_device_id + j);
                sh100_ptr_agv_[j] = sros::device::createDevice<sros::device::SH100>(
                ...
                auto func = std::bind(&Sh100SensorManager::onDataRecieve, this, sh100_id, std::placeholders::_1,
                                      std::placeholders::_2, std::placeholders::_3);
                // get and set device info
                sh100_ptr_agv_[j]->setModelNo( sh100_ptr_agv_[j]->getDeviceModelStr() );
                sh100_ptr_agv_[j]->setVersionNo( sh100_ptr_agv_[j]->getDeviceVersionStr() );
                sh100_ptr_agv_[j]->setSerialNo("-");
                for (int i = 0; i < each_sh100_tof_num; ++i) {
                    auto tof_id = j * each_sh100_tof_num + i;
                    if (tof_id < para->tof_nums) {
                      if(para->sh100_tof_enable_states[tof_id]){
                        ...
                        tof_devices_[tof_id]->setModelNo("-");
                        tof_devices_[tof_id]->setVersionNo( sh100_ptr_agv_[j]->getVersionNo() );
                        tof_devices_[tof_id]->setSerialNo("-");
                      }
                    }else{
                ...
                //初始化完所有TOF后再设置回调，防止数据过早回调导致设备取空异常
                sh100_ptr_agv_[j]->setTofDataCallback(func);
            }
        }
    }
 ...
 private:
    ...
    void onDataRecieve(int device_id, uint8_t id, uint16_t distance, uint16_t strength) {
        ...
        if (tof_array_index < para->tof_nums) {
            ...
            tof_base_coord = sh100_tfs_[tof_array_index] * Eigen::Vector2d(tof_dist, 0.0);
    ...
    std::vector<Eigen::Affine2d> sh100_tfs_;
    std::vector<sros::device::Device_ptr> tof_devices_;
    std::string oba_name_;
    std::vector<sros::device::SH100_ptr> sh100_ptr_agv_;
};
