...
# if EMBEDDED != 1
  c_int   rho_updates;  ///< number of osqp_rho updates
  c_float rho_estimate; ///< best osqp_rho estimate so far from residuals
# endif // if EMBEDDED != 1
} OSQPInfo;
 ...
 */
typedef struct {
  c_float osqp_rho;                    ///< ADMM step osqp_rho
  c_float sigma;                  ///< ADMM step sigma
  c_int   scaling;                ///< heuristic data scaling iterations; if 0, then disabled.

# if EMBEDDED != 1
  c_int   adaptive_rho;           ///< boolean, is osqp_rho step size adaptive?
  c_int   adaptive_rho_interval;  ///< number of iterations between osqp_rho adaptations; if 0, then it is automatic
  c_float adaptive_rho_tolerance; ///< tolerance X for adapting osqp_rho. The new osqp_rho has to be X times larger or 1/X times smaller than the current one to trigger a new factorization.
#  ifdef PROFILING
  c_float adaptive_rho_fraction;  ///< interval for adapting osqp_rho (fraction of the setup time)
#  endif // Profiling
# endif // EMBEDDED != 1

  c_int                   osqp_max_iter;      ///< maximum number of iterations
...
# endif // ifndef EMBEDDED
  ...
  /**
   * @name Vector used to store a vectorized osqp_rho parameter
   * @{
   */
  c_float *rho_vec;     ///< vector of osqp_rho values
  c_float *rho_inv_vec; ///< vector of inv osqp_rho values
...
# endif // if EMBEDDED != 1
   ...
   * approximate tolerances computation and adapting osqp_rho
   * @{
   */
