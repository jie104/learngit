...
bool Connection::connect(const char* server_ip, int server_port) {
    ...
    try {
        ...
        // wait for the first msg
//        auto m = receiveMsg<StateMsg>();
//        _handle_msg(m);
 ...
 * 阻塞方法
 */
bool Connection::sendMsg(connection::BaseMsg_ptr msg) {
    if (ec_)
        return false;

    msg->encode();
    bool r = _write_some_bytes(msg->rawData().data(), msg->getLength());
...
// 使用udp发送pose数据
bool Connection::sendUDPMsg(connection::BaseMsg_ptr msg) {
    if (!enable_udp_) return false;

    msg->encode();
    return udp_socket_.send_to(boost::asio::buffer(msg->rawData().data(), msg->getLength()),
                               udp_endpoint_) > 0;
}
 ...
 */
connection::BaseMsg_ptr Connection::_read_msg() {
    if (ec_)
        return NULL;

    if (!_read_some_bytes((unsigned char *) header_msg_.rawData().data(),
                          header_msg_.getHeaderLength())) {
        return NULL;
    }
    header_msg_.decode();
    auto header_length = header_msg_.getHeaderLength();
    auto body_length = header_msg_.getBodyLength();
    connection::BaseMsg_ptr msg = connection::ConnectMsgFactory::getMsg(header_msg_.getType());
    auto &raw_data = msg->rawData();
    raw_data = header_msg_.rawData();
    raw_data.resize(header_length + body_length);
    if (!_read_some_bytes((unsigned char *) msg->rawData().data()+ header_length, body_length)) {
        return NULL;
    }
    msg->decode();
    return msg;
 ...
 */
void Connection::_handle_msg(connection::BaseMsg_ptr read_msg) {
...
void Connection::close() {
    ...
    udp_recv_pose_thread_running_ = false;
    udp_socket_.close();
}
