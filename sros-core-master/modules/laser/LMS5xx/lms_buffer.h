...
#ifndef LMS5XX_LMS_BUFFER_H_
#define LMS5XX_LMS_BUFFER_H_

#include <stdint.h>
#include <string.h>
#include <unistd.h>
#include <glog/logging.h>
#define LMS_BUFFER_SIZE 100000
#define LMS_STX 0x02
#define LMS_ETX 0x03
namespace lms5xx{
...
{
public:
    LMSBuffer() : total_length_(0), end_of_first_message_(0), buffer_(NULL) {
      buffer_ = new char[LMS_BUFFER_SIZE];
    }
    ...
    {
      int ret = read(fd, buffer_ + total_length_, LMS_BUFFER_SIZE - total_length_);
...
//      logDebug("Empty buffer, nothing to return.");
      LOG(INFO) << "the total length is zero!";
      return NULL;
    }
    ...
    if (start_of_message == NULL) {
      ...
      LOG(INFO) << "No STX found, dropping %d bytes from buffer." << total_length_;
      total_length_ = 0;
    }
    else if (buffer_ != start_of_message) {
      // Start of message found, ahead of the start of buffer. Therefore shift the buffer back.
      LOG(INFO) << "Shifting buffer, dropping %d bytes, %d bytes remain." <<
