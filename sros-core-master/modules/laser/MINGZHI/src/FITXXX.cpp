...
//根据头文件地址，解析数据到scan_data
bool FITXXX::LIM_CODE_LMD_Decoding(LIM_HEAD* lim, sros::core::LaserScanMsg *scan_data)
{
  if (lim->nCode != LIM_CODE_LMD)
    return false;
  LOG(INFO) << "-------------" << lim->Data[2] << "." << lim->Data[3];
  uint64_t time = ((lim->Data[2] * 1000) + lim->Data[3]) * 1000;//us
  scan_data->time_ = time;
  LMD_INFO* lmd_info = LMD_Info(lim); //info
  LMD_D_Type* lmd = LMD_D(lim);  //data
...
bool FITXXX::GetALim(ULDINI_Type *uld, sros::core::LaserScanMsg *scan_data)
{
  ...
  if (buffer_data)
  {
    ...
    if (LIM_CODE_LDBCONFIG == lim->nCode)//设备配置信息
    {
      if(LIM_CODE_LDBCONFIG_Decoding(lim, uld))
      {
        loaded_config_ = true;
        scan_data->time_ = (lim->Data[2]*1000+lim->Data[3])*1000;//虽然不返回有效数据，但是要返回有效时间
      }
    }

    if ((LIM_CODE_LMD == lim->nCode)&&(loaded_config_==true))//测量数据报文
    {
        if(!LIM_CODE_LMD_Decoding(lim, scan_data)){
            return false;
        }
    }
