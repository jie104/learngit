...
int HttpCommandInterface::httpGet(const std::string request_path, std::string &header, std::string &content)
{
    ...
    try
    {
        ...
        {
            socket.close();
            system("sysctl net.ipv4.tcp_syn_retries=1");
            socket.connect(*endpoint_iterator++, error);
            system("sysctl net.ipv4.tcp_syn_retries=6");
        }
        if (error)
...
bool HttpCommandInterface::sendHttpCommand(const std::string cmd, const std::map<std::string, std::string> param_values)
{
    ...
    try
    {
        if (http_status_code_ != 200) {
            LOG(INFO) << "status code is wrong!" << header << "," << content;
            return false;
        }
        if (content.empty()) {
            std::cerr << "ERROR: Exception: content is empty!";
        }
        std::stringstream ss(content);
//        LOG(INFO) << "content:" << content << "header:" << header;
        boost::property_tree::ptree tmp_pt = pt_;
