...
bool LMS1xx::parseScanData(char* buffer, scanData* data)
{
...
//    printf("NumberData : %d,type is: %d\n", NumberData, type);
  ...
  for (int i = 0; i < NumberChannels8Bit; i++)
  {
    ...
    sscanf(tok, "%X", &NumberData);
    printf("the second number data is:%d\n", NumberData);
    if (NumberData != scan_number_check) {
      return false;
    }
    if (type == 0)
...
void LMS1xx::setScanNumberCheck(int number_check) {
  scan_number_check = number_check;
}

bool LMS1xx::readTillEnd(){
  char buf[1];
  bool is_end = false;
  int read_count = 0;
  while (!is_end) {
