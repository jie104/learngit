...

{
  ...
  {
    LOG(INFO)<<"Exception while creating socket: %s", e.what();
  }
  m_remote_endpoint = boost::asio::ip::tcp::endpoint(server_ip, server_port);
    LOG(INFO)<<"TCP client is setup";
}
...
void AsyncTCPClient::doDisconnect()
{
  ...
  if (ec != 0)
  {
      LOG(WARNING) << "Error shutting socket down: " << ec.value();
  }
  else
  {
    LOG(INFO)<<"TCP Connection successfully shutdown";
  }

  m_socket_ptr->close(ec);
  if (ec != 0)
  {
    LOG(INFO)<<"Error closing Socket: %i", ec.value();
  }
  else
  {
      LOG(INFO)<<"TCP Socket successfully closed.";
  }
}
...
bool AsyncTCPClient::doConnect()
{
  ...
  m_socket_ptr->async_connect(m_remote_endpoint, [&](boost::system::error_code ec) {
        if (ec != 0) {
            LOG(INFO) << "TCP error code: " << ec.value();
        } else {
            LOG(INFO) << ("TCP connection successfully established.");
            is_conected = true;
        }
                                          ...
                                          const std::size_t& bytes_transferred)
{
  ...
  {
      LOG(INFO) << "Error in tcp handle send and receive: " << error.value();
  }
}
                                   ...
                                   const std::size_t& bytes_transferred)
{
  ...
  {
      LOG(INFO)<<"Error in tcp handle receive:"<<error.value();
  }
}
