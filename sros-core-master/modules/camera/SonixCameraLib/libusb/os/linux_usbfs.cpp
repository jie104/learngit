...
static int discard_urbs(struct usbi_transfer *itransfer, int first, int last_plus_one)
{
	...
	struct linux_transfer_priv *tpriv =
            (linux_transfer_priv*)usbi_transfer_get_os_priv(itransfer);
	...
	unsigned char urb_type)
{
	struct libusb_transfer *transfer =
		USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
	struct linux_transfer_priv *tpriv = (linux_transfer_priv*)usbi_transfer_get_os_priv(itransfer);
	struct linux_device_handle_priv *dpriv =
		_device_handle_priv(transfer->dev_handle);
	struct usbfs_urb *urbs;
	...
	urbs = (usbfs_urb*)malloc(alloc_size);
	if (!urbs)
		return LIBUSB_ERROR_NO_MEM;
...
static int submit_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	for (i = 0; i < num_urbs; i++) {
		...
		urb = (usbfs_urb*)malloc(alloc_size);
		if (!urb) {
			free_iso_urbs(tpriv);
			return LIBUSB_ERROR_NO_MEM;
		}
...
static int submit_control_transfer(struct usbi_transfer *itransfer)
{
	...
	urb = (usbfs_urb*)malloc(sizeof(struct usbfs_urb));
	if (!urb)
		return LIBUSB_ERROR_NO_MEM;
	...
	struct usbfs_urb *urb)
{
	...
	tpriv->urbs = NULL;
	usbi_mutex_unlock(&itransfer->lock);
	return usbi_handle_transfer_completion(itransfer, (libusb_transfer_status)status);
}
...
static int reap_for_handle(struct libusb_device_handle *handle)
{
	...
	itransfer = (usbi_transfer*)urb->usercontext;
