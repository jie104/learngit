...
// LIBUSB_DESCRIBE="";
//const struct libusb_version libusb_version_internal = {LIBUSB_MAJOR, LIBUSB_MINOR, LIBUSB_MICRO,
//                                                       LIBUSB_NANO,  LIBUSB_RC,    LIBUSB_DESCRIBE};

struct libusb_context *svc100_usbi_default_context = NULL;
static int default_context_refcnt = 0;
static usbi_mutex_static_t svc100_default_context_lock = USBI_MUTEX_INITIALIZER;
...
static struct discovered_devs *discovered_devs_alloc(void) {
    struct discovered_devs *ret =
        (discovered_devs *)malloc(sizeof(*ret) + (sizeof(void *) * DISCOVERED_DEVICES_SIZE_STEP));
...
struct discovered_devs *discovered_devs_append(struct discovered_devs *discdevs, struct libusb_device *dev) {
    ...
    discdevs = (discovered_devs *)realloc(discdevs, sizeof(*discdevs) + (sizeof(void *) * capacity));
    if (discdevs) {
...
static void discovered_devs_free(struct discovered_devs *discdevs) {
    ...
    for (i = 0; i < discdevs->len; i++) libusb_unref_device(discdevs->devices[i]);
...
struct libusb_device *usbi_alloc_device(struct libusb_context *ctx, unsigned long session_id) {
 ...
 * NULL otherwise. */
struct libusb_device *usbi_get_device_by_session_id(struct libusb_context *ctx, unsigned long session_id) {
...
ssize_t API_EXPORTED libusb_get_device_list(libusb_context *ctx, libusb_device ***list) {
    ...
    if (!discdevs) return LIBUSB_ERROR_NO_MEM;
 ...
 */
void API_EXPORTED libusb_free_device_list(libusb_device **list, int unref_devices) {
    if (!list) return;
    ...
    if (unref_devices) {
        ...
        while ((dev = list[i++]) != NULL) libusb_unref_device(dev);
    }
    free(list);
}
 ...
 */
uint8_t API_EXPORTED libusb_get_bus_number(libusb_device *dev) { return dev->bus_number; }
 ...
 */
uint8_t API_EXPORTED libusb_get_device_address(libusb_device *dev) { return dev->device_address; }
 ...
 */
int API_EXPORTED libusb_get_device_speed(libusb_device *dev) { return dev->speed; }

static const struct libusb_endpoint_descriptor *find_endpoint(struct libusb_config_descriptor *config,
 ...
 */
int API_EXPORTED libusb_get_max_packet_size(libusb_device *dev, unsigned char endpoint) {
    ...
    if (!ep) return LIBUSB_ERROR_NOT_FOUND;
 ...
 * \returns LIBUSB_ERROR_OTHER on other failure
 */
int API_EXPORTED libusb_get_max_iso_packet_size(libusb_device *dev, unsigned char endpoint) {
    struct libusb_config_descriptor *config;
    const struct libusb_endpoint_descriptor *ep;
...
void API_EXPORTED libusb_unref_device(libusb_device *dev) {
    ...
    if (refcnt == 0) {
        ...
        if (usbi_backend->destroy_device) usbi_backend->destroy_device(dev);
...
void usbi_fd_notification(struct libusb_context *ctx) {
    ...
    if (r <= 0) usbi_warn(ctx, "internal signalling read failed");
...
int API_EXPORTED libusb_open(libusb_device *dev, libusb_device_handle **handle) {
    ...
    if (!_handle) return LIBUSB_ERROR_NO_MEM;
...
DEFAULT_VISIBILITY
libusb_device_handle *LIBUSB_CALL libusb_open_device_with_vid_pid(libusb_context *ctx, uint16_t vendor_id,
                                                                  uint16_t product_id) {
    ...
    if (libusb_get_device_list(ctx, &devs) < 0) return NULL;
...
static void do_close(struct libusb_context *ctx, struct libusb_device_handle *dev_handle) {
    ...
    list_for_each_entry_safe(itransfer, tmp, &ctx->flying_transfers, list, struct usbi_transfer) {
        ...
        if (transfer->dev_handle != dev_handle) continue;
         ...
         */
        usbi_dbg("Removed transfer %p from the in-flight list because device handle %p closed", transfer, dev_handle);
    }
...
void API_EXPORTED libusb_close(libusb_device_handle *dev_handle) {
    ...
    /* read the dummy data */
    r = usbi_read(ctx->ctrl_pipe[0], &dummy, sizeof(dummy));
    if (r <= 0) usbi_warn(ctx, "internal signalling read failed, closing anyway");
 ...
 */
DEFAULT_VISIBILITY
libusb_device *LIBUSB_CALL libusb_get_device(libusb_device_handle *dev_handle) { return dev_handle->dev; }
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED libusb_get_configuration(libusb_device_handle *dev, int *config) {
    int r = LIBUSB_ERROR_NOT_SUPPORTED;

    usbi_dbg("");
    if (usbi_backend->get_configuration) r = usbi_backend->get_configuration(dev, config);

    if (r == LIBUSB_ERROR_NOT_SUPPORTED) {
        uint8_t tmp = 0;
        usbi_dbg("falling back to control message");
        r = libusb_control_transfer(dev, LIBUSB_ENDPOINT_IN, LIBUSB_REQUEST_GET_CONFIGURATION, 0, 0, &tmp, 1, 1000);
        if (r == 0) {
    ...
    if (r == 0) usbi_dbg("active config %d", *config);
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED libusb_set_configuration(libusb_device_handle *dev, int configuration) {
...
int API_EXPORTED libusb_claim_interface(libusb_device_handle *dev, int interface_number) {
    ...
    if (interface_number >= USB_MAXINTERFACES) return LIBUSB_ERROR_INVALID_PARAM;

    usbi_mutex_lock(&dev->lock);
    if (dev->claimed_interfaces & (1 << interface_number)) goto out;

    r = usbi_backend->claim_interface(dev, interface_number);
    if (r == 0) dev->claimed_interfaces |= 1 << interface_number;
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED libusb_release_interface(libusb_device_handle *dev, int interface_number) {
    int r;
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED libusb_set_interface_alt_setting(libusb_device_handle *dev, int interface_number,
    ...
    return usbi_backend->set_interface_altsetting(dev, interface_number, alternate_setting);
}
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED libusb_clear_halt(libusb_device_handle *dev, unsigned char endpoint) {
 ...
 */
int API_EXPORTED libusb_kernel_driver_active(libusb_device_handle *dev, int interface_number) {
    usbi_dbg("interface %d", interface_number);
 ...
 */
int API_EXPORTED libusb_detach_kernel_driver(libusb_device_handle *dev, int interface_number) {
    usbi_dbg("interface %d", interface_number);
 ...
 * \see libusb_kernel_driver_active()
 */
int API_EXPORTED libusb_attach_kernel_driver(libusb_device_handle *dev, int interface_number) {
    usbi_dbg("interface %d", interface_number);
...
void API_EXPORTED libusb_set_debug(libusb_context *ctx, int level) {
    USBI_GET_CONTEXT(ctx);
    if (!ctx->debug_fixed) ctx->debug = level;
}
...
int API_EXPORTED libusb_init(libusb_context **context) {
    ...
    usbi_mutex_static_lock(&svc100_default_context_lock);
    if (!context && svc100_usbi_default_context) {
        usbi_dbg("reusing default context");
        default_context_refcnt++;
        usbi_mutex_static_unlock(&svc100_default_context_lock);
        return 0;
    }
    ...
    if (dbg) {
        ctx->debug = atoi(dbg);
        if (ctx->debug) ctx->debug_fixed = 1;
    }

    usbi_dbg("libusb-%d.%d.%d%s%s%s", libusb_version_internal.major, libusb_version_internal.minor,
             libusb_version_internal.micro, libusb_version_internal.rc,
             libusb_version_internal.describe[0] ? " git:" : "", libusb_version_internal.describe);

    if (usbi_backend->init) {
        r = usbi_backend->init(ctx);
        if (r) goto err_free_ctx;
    }
    ...
    if (context) {
        *context = ctx;
    } else if (!svc100_usbi_default_context) {
        usbi_dbg("created default context");
        svc100_usbi_default_context = ctx;
        default_context_refcnt++;
    }
    usbi_mutex_static_unlock(&svc100_default_context_lock);
...
void API_EXPORTED libusb_exit(struct libusb_context *ctx) {
    ...
    if (ctx == svc100_usbi_default_context) {
        usbi_mutex_static_lock(&svc100_default_context_lock);
        ...
        svc100_usbi_default_context = NULL;
        usbi_mutex_static_unlock(&svc100_default_context_lock);
    }
    ...
    if (!list_empty(&ctx->open_devs)) usbi_warn(ctx, "application left some devices open");
...
int API_EXPORTED libusb_has_capability(uint32_t capability) {
    enum libusb_capability cap = (libusb_capability)capability;
    switch (cap) {
...
void usbi_log_v(struct libusb_context *ctx, enum usbi_log_level level, const char *function, const char *format,
                va_list args) {
    FILE *stream = stdout;
    const char *prefix;
    struct timeval now;
    ...
    USBI_GET_CONTEXT(ctx);
    if (!ctx->debug) return;
    if (level == LOG_LEVEL_WARNING && ctx->debug < 2) return;
    if (level == LOG_LEVEL_INFO && ctx->debug < 3) return;
#endif
    ...
    fprintf(stream, "libusb: %d.%06d %s [%s] ", (int)now.tv_sec, (int)now.tv_usec, prefix, function);
...
void usbi_log(struct libusb_context *ctx, enum usbi_log_level level, const char *function, const char *format, ...) {
    va_list args;
 ...
 */
//DEFAULT_VISIBILITY
//const struct libusb_version *LIBUSB_CALL libusb_get_version(void) { return &libusb_version_internal; }
}