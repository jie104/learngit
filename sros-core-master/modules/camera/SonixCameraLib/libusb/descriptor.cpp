	...
	int host_endian)
{
	...
	while (size >= INTERFACE_DESC_LENGTH) {
		...
		altsetting = (libusb_interface_descriptor*)realloc(altsetting,
		...
		len = (int)(buffer - begin);
		if (len) {
			ifp->extra = (const unsigned char*)malloc(len);
			if (!ifp->extra) {
				r = LIBUSB_ERROR_NO_MEM;
				goto err;
			}
		...
		if (ifp->bNumEndpoints > 0) {
			...
			endpoint = (libusb_endpoint_descriptor*)malloc(tmp);
	...
	int host_endian)
{
	...
	usb_interface = (libusb_interface*)malloc(tmp);
	...
	for (i = 0; i < config->bNumInterfaces; i++) {
		...
		if (len) {
			...
			if (!config->extra_length) {
				config->extra = (const unsigned char*)malloc(len);
				if (!config->extra) {
					r = LIBUSB_ERROR_NO_MEM;
					goto err;
				}
...
{
	struct libusb_config_descriptor *_config = (libusb_config_descriptor*)malloc(sizeof(*_config));
	unsigned char tmp[8];
	...
	buf = (unsigned char*)malloc(_config->wTotalLength);
	if (!buf) {
		r = LIBUSB_ERROR_NO_MEM;
		goto err;
	}
	...
	uint8_t config_index, struct libusb_config_descriptor **config)
{
	...
	_config = (libusb_config_descriptor*)malloc(sizeof(*_config));
	if (!_config)
		return LIBUSB_ERROR_NO_MEM;
