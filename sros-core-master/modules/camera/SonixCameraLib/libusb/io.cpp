	...
	int iso_packets)
{
	...
	size_t alloc_size = sizeof(struct usbi_transfer)
		...
		+ os_alloc_size;
	struct usbi_transfer *itransfer = (usbi_transfer*)malloc(alloc_size);
	if (!itransfer)
		return NULL;
...
static int handle_events(struct libusb_context *ctx, struct timeval *tv)
{
	...
	fds = (pollfd*)malloc(sizeof(*fds) * nfds);
	if (!fds) {
	...
	libusb_context *ctx)
{
	...
	ret = (libusb_pollfd**)calloc(cnt + 1, sizeof(struct libusb_pollfd *));
	if (!ret)
		goto out;
