...
bool V4L2Camera::open_device(int index) {
    ...
    std::string dev_name = "/dev/video" + std::to_string(index);  // "/dev/v4l/by-id/usb-Sonix_Technology_Co.__Ltd._down-video-index0";
...
double V4L2Camera::getImageProp(int property) {
    return 0.0;
...
bool V4L2Camera::printPixelFormat() {
    ...
    if(ioctl(fd_, VIDIOC_G_FMT, &fmt) == -1){
        ...
        LOG(INFO) << "fmt pixel format is :" << (fmt.fmt.pix.pixelformat & 0xFF,(fmt.fmt.pix.pixelformat >> 8) & 0xFF,
...
bool V4L2Camera::isGrabed() {
    return is_grabed_;
}

bool V4L2Camera::readFrame(cv::Mat &frame, bool needDecode) {
    if(!isOpened()){
...
bool V4L2Camera::createBuffers() {
    ...
    for(n_buffers = 0; n_buffers < req.count; n_buffers++){
        ...
        buffers_[n_buffers].start = (uint8_t *)mmap(NULL,
                                                    buf.length,
...
int V4L2Camera::getBandwidth() {

    return 0;
}

bool V4L2Camera::getUsbBus(std::string &bus) {
    ...
    if(isOpened()){
        ...
        bus = std::string((char*)(cap.bus_info));
        return true;
    }
