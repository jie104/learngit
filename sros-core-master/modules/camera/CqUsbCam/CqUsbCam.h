...
#include "tagSensor.h"
 ...
 public:
	...
	cq_int32_t WrEeprom(const cq_uint32_t iAddr, const uint8_t* buffer,cq_uint32_t length);
	cq_int32_t RdEeprom(const cq_uint32_t iAddr, cq_uint8_t * buffer, cq_uint32_t &length);

	int32_t ResetFlag(void);
	int32_t ResetDevice(const int32_t usbNum);
};
