...
static cq_int32_t MT9V034_ArbitrFunc(cyusb_handle *pUsbHandle, void* arg)
{
	...
	{
		cyusb_control_write(pUsbHandle, 0x40, arbfunc.order.ReqCode, arbfunc.order.Value, arbfunc.order.Index, arbfunc.order.pData, 2, 1000);
	
	}
	else
	{

		cyusb_control_read(pUsbHandle,0x40,arbfunc.order.ReqCode,arbfunc.order.Value,arbfunc.order.Index,arbfunc.order.pData,2,1000);
