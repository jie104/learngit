...
std::unordered_map<int, std::size_t> PIX_SZ{
    {CV_8U, 1},  {CV_8S, 1},  {CV_16U, 2}, {CV_16S, 2},
    {CV_32S, 4}, {CV_32F, 4}, {CV_64F, 8}};
...
class ImageBuffer::Impl {
 ...
 protected:
  ...
  template <typename T>
  void cloud_create(cv::Mat& im, pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
                    std::uint32_t fmt, std::size_t xidx, std::size_t yidx,
                    std::size_t zidx, std::uint32_t width, std::uint32_t height,
                    std::uint32_t npts,
  ...
  void set_cloud_intensity(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
    ...
    if (cloud->points.size() != numpts) {
      VLOG(IFM3D_PROTO_DEBUG) << "Shape mismatch when coloring pcl intensity: "
...
// Accessors
//-------------------------------------
cv::Mat ifm3d::ImageBuffer::Impl::DistanceImage() { return this->dist_; }

cv::Mat ifm3d::ImageBuffer::Impl::UnitVectors() { return this->uvec_; }

cv::Mat ifm3d::ImageBuffer::Impl::GrayImage() { return this->gray_; }

cv::Mat ifm3d::ImageBuffer::Impl::AmplitudeImage() { return this->amp_; }

cv::Mat ifm3d::ImageBuffer::Impl::RawAmplitudeImage() { return this->ramp_; }

cv::Mat ifm3d::ImageBuffer::Impl::ConfidenceImage() { return this->conf_; }

cv::Mat ifm3d::ImageBuffer::Impl::XYZImage() { return this->xyz_; }

pcl::PointCloud<pcl::PointXYZI>::Ptr ifm3d::ImageBuffer::Impl::Cloud() {
  return this->cloud_;
}

std::vector<float> ifm3d::ImageBuffer::Impl::Extrinsics() {
  return this->extrinsics_;
}

std::vector<std::uint32_t> ifm3d::ImageBuffer::Impl::ExposureTimes() {
  return this->exposure_times_;
}

ifm3d::TimePointT ifm3d::ImageBuffer::Impl::TimeStamp() {
  return this->time_stamp_;
}

float ifm3d::ImageBuffer::Impl::IlluTemp() { return this->illu_temp_; }
//-------------------------------------
// Looping over the pixel bytes
//-------------------------------------

void ifm3d::ImageBuffer::Impl::Organize(
    const std::vector<std::uint8_t>& bytes) {
  ...
  extidx =
      ifm3d::get_chunk_index(bytes, ifm3d::image_chunk::EXTRINSIC_CALIBRATION);
  ...
  const std::uint32_t header_version =
      ifm3d::mkval<std::uint32_t>(bytes.data() + cidx + 12);
  ...
  std::uint32_t xfmt =
      CART_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + xidx + 24)
              : INVALID_FMT;
  std::uint32_t yfmt =
      CART_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + yidx + 24)
              : INVALID_FMT;
  std::uint32_t zfmt =
      CART_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + zidx + 24)
              : INVALID_FMT;
  std::uint32_t afmt =
      A_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + aidx + 24)
           : INVALID_FMT;
  std::uint32_t raw_afmt =
      RAW_A_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + raw_aidx + 24)
               : INVALID_FMT;
  std::uint32_t dfmt =
      D_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + didx + 24)
           : INVALID_FMT;
  std::uint32_t ufmt =
      U_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + uidx + 24)
           : INVALID_FMT;
  std::uint32_t extfmt =
      EXT_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + extidx + 24)
             : INVALID_FMT;
  std::uint32_t gfmt =
      G_OK ? ifm3d::mkval<std::uint32_t>(bytes.data() + gidx + 24)
           : INVALID_FMT;
  ...
  std::uint32_t width = ifm3d::mkval<std::uint32_t>(bytes.data() + cidx + 16);
  std::uint32_t height = ifm3d::mkval<std::uint32_t>(bytes.data() + cidx + 20);
  std::uint32_t npts = width * height;

  VLOG(IFM3D_PROTO_DEBUG) << "npts=" << npts << ", width x height=" << width
                          << " x " << height;

  auto im_wrapper = [this, width, height, npts, &bytes](
                        cv::Mat& im, std::uint32_t fmt, std::size_t idx) {
    switch (fmt) {
      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_8U):
        this->im_create<std::uint8_t>(im, fmt, idx, width, height, 1, npts,
                                      bytes);
        break;

      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_8S):
        this->im_create<std::int8_t>(im, fmt, idx, width, height, 1, npts,
                                     bytes);
        break;

      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_16U):
        this->im_create<std::uint16_t>(im, fmt, idx, width, height, 1, npts,
                                       bytes);
        break;

      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_16S):
        this->im_create<std::int16_t>(im, fmt, idx, width, height, 1, npts,
                                      bytes);
        break;

      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_32S):
        this->im_create<std::int32_t>(im, fmt, idx, width, height, 1, npts,
                                      bytes);
        break;

      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_32F):
        this->im_create<float>(im, fmt, idx, width, height, 1, npts, bytes);
        break;

      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_32F3):
        this->im_create<float>(im, fmt, idx, width, height, 3, npts, bytes);
        break;

      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_64F):
        this->im_create<double>(im, fmt, idx, width, height, 1, npts, bytes);
        break;
  ...
  auto cloud_wrapper = [this, width, height, npts, &bytes](
                           cv::Mat& im,
                           pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
                           std::uint32_t fmt, std::size_t xidx,
                           std::size_t yidx, std::size_t zidx) {
    switch (fmt) {
      case static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_16S):
  ...
  if (CART_OK) {
    ...
    if (A_OK) {
      ...
      switch (depth) {
        ...
        case CV_16U:
          this->set_cloud_intensity<std::uint16_t>(this->cloud_, this->amp_);
          break;
  ...
  if (EXT_OK) {
    ...
    if (extfmt != static_cast<std::uint32_t>(ifm3d::pixel_format::FORMAT_32F)) {
      LOG(ERROR) << "Extrinsics are expected to be float32, not: " << extfmt;
      throw ifm3d::error_t(IFM3D_PIXEL_FORMAT_ERROR);
    }
  ...
  if (EXT_OK) {
    ...
    if (bytes_left >= 18 &&
        std::equal(bytes.begin() + extidx, bytes.begin() + extidx + 6,
                   std::begin("extime"))) {
      extime_idx += 6;
      bytes_left -= 6;
      ...
      for (std::size_t i = 0; i < 3; ++i) {
        ...
        std::uint32_t extime2 =
            ifm3d::mkval<std::uint32_t>(bytes.data() + extime_idx);
      ...
      std::fill(this->exposure_times_.begin(), this->exposure_times_.end(), 0);
    }

    // Read temp_illu (9 bytes string + 4 bytes float)
    if (bytes_left >= 13 &&
        std::equal(bytes.begin() + extidx, bytes.begin() + extidx + 8,
                   std::begin("temp_illu"))) {
      extime_idx += 9;
      bytes_left -= 9;

      this->illu_temp_ = ifm3d::mkval<float>(bytes.data() + extime_idx);
...
//    LOG(WARNING) << "Checking for illu temp and exposure times skipped (cant "
//                    "trust extidx)";
  }
}
