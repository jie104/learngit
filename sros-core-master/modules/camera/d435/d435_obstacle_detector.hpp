...
#ifndef SROS_D435_OBSTACLE_DETECTOR_HPP
#define SROS_D435_OBSTACLE_DETECTOR_HPP
#include <Eigen/Dense>
...
struct PlaneModelInfo {
    std::vector<PlaneInfo_Ptr> infos;
    PlaneInfo info;
};

class D435ObstacleDetector {

 public:

    D435ObstacleDetector() {
    ...
    virtual ~D435ObstacleDetector(){

    }

    //欧拉角自转右乘
    static Eigen::Matrix3d eulerAngelToRotationMatrix(const double &x_rad, const double &y_rad, const double &z_rad){
        Eigen::Matrix3d x_mat,y_mat,z_mat, rot_mat;
        x_mat <<1,            0,                          0,
                0,            cos(x_rad),      -sin(x_rad),
                0,            sin(x_rad),       cos(x_rad);

        y_mat<<cos(y_rad),          0,      sin(y_rad),
                0,                     1,              0,
                -sin(y_rad),          0,       cos(y_rad);

        z_mat<< cos(z_rad), -sin(z_rad),    0,
                sin(z_rad),  cos(z_rad),    0,
                0,              0,             1;

        rot_mat = x_mat * y_mat * z_mat;
        LOG(INFO)<<"DstMat:   "<<rot_mat;
        return rot_mat;
    }
    ...
    bool calibrateCamera(rs2::points& points, int length, int width, Eigen::Matrix3f& install_matrix,
...
//                    }
                    ...
                    // Modify by zjWu  考虑标定平面抬高或抬高叉车的情况，抬高叉车相当于降低标定平面高度
                    float compute_install_height;
                    if (pitch > 0) {
                        compute_install_height = -mean[2] + plane_height;  //标定平面可能高于地平面或低于地平面
                    } else {
                        compute_install_height = -mean[2] - plane_height;  //传感器出射方向朝上，标定平面高度为负值
                        if (plane_height < -0.1f) {
                            compute_install_height -= 0.01f;    //做个补偿。
                        }
                    }
    ...
    bool getEnoughtPlane(rs2::points& points, int length, int width){
        serialization::PointImage point_image(length, width);
        LOG(INFO) << "calibrate!";
        computePointImage(points, length, width, point_image);
        LOG(INFO) << "calibrate!" << length << "," << width;
    ...
    void computeObstaclePoints(rs2::points& points, int length, int width, StereoPoints& oba_points,int obszie_) {
        high_coarse_map_->clear();
        low_coarse_map_->clear();
        ground_map_->clear();
        buildLocalMap(points, length, width);
        Eigen::Quaternionf initial_quat_offset;
        EulerToQuaternion(0, 0, -3.0 * M_PI / 180.0, initial_quat_offset);//将D435故意向下倾斜，来使远处的特征高度阈值变大。
        ...
        for (int l = 0; l < width_int; ++l) {
            for (int i = 0; i < length_int; ++i) {
                ...
                if(!high_cell.empty()){
                    ...
                    if (sum_size >= obszie_) {
                        for (auto &high : high_cell) {
                            ...
                            high[2] -= install_height_;
                            high = initial_quat_offset * high;
                            high[2] += install_height_;
                ...
                if (low_cell.size() >= 3) {
                    ...
                    if (real_points.size() >= obszie_) {
                        for(auto& point:real_points){
                            oba_points.emplace_back();
                            point[2] -= install_height_;
                            point = initial_quat_offset * point;
                            point[2] += install_height_;
    ...
    void buildLocalMap(rs2::points& rs_points, int length, int width){
    ...
    void computePointImage(rs2::points& rs_points, int length, int width, serialization::PointImage& image) {
        auto vertices = rs_points.get_vertices();
...
#endif  // SROS_D435_OBSTACLE_DETECTOR_HPP
