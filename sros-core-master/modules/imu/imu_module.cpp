...
namespace imu
{
    ...
    // SensorHandle *ImuModule::imu_handle_ = nullptr;
    std::unique_ptr<SensorHandle> ImuModule::imu_handle_(nullptr);
    ...
    void ImuModule::run()
    {
        ...
        imu_handle_.reset(new SensorHandle(imu_type, imu_dev, imu_frequency));
        imu_handle_->setExtTransform(imu_roll, imu_pitch, imu_yaw, imu_trans_x, imu_trans_y, imu_trans_z);
    ...
    {
        if (imu_handle_)
            return imu_handle_->getImuWithIdx(index);
        else
            return ImuData{};
    }
    ...
    {
        if (imu_handle_)
            return imu_handle_->getImuWithStamp(stamp);
        else
            return ImuData{};
    }
    ...
    {
        if (imu_handle_)
            return imu_handle_->setStandstill(is_standstill);
    }

    std::shared_ptr<SensorHandle> generateImuHandle(const int imu_type)
    {
        static std::shared_ptr<SensorHandle> imu_handle = std::make_shared<SensorHandle>(imu_type);
        return imu_handle;
    }
