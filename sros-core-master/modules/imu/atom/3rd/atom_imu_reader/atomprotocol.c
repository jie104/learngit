...
int AtomCmd_Compose_Send(u8 MADDR, u8 ClassID, u8 msgID, u8* payloadBuffer, u16 PayloadLen) {
    ...
    // Payload
    for (int i = 0; i < PayloadLen; i++) pBuf[index++] = payloadBuffer[i];
    ...
    printf("TX: ");
    for (int i = 0; i < (ATOM_CMDLEN_NO_PAYLOAD + PayloadLen); i++) printf("%02X ", pBuf[i]);

    printf("\n");

    SendUARTMessageLength((const char*)pBuf, index);
...
int checkCIDValidation(u8 classID) {
    int ret = 0;
    if (!((classID == 0x01) || (classID == 0x02) || (classID == 0x03) || (classID == 0x04) || (classID == 0x05) ||
          (classID == 0x06) || (classID == 0x0A) || (classID == 0x0E))) {
        ret = 0;
    } else
        ret = 1;
        ...
        if (((*pCurrent)[processInd] == FRAME_PREAMBLE1) && (((*pCurrent)[processInd + 1] == FRAME_PREAMBLE2))) {
            ...
            if (!ret) {
                ...
                *punProcessedLen = len;
                ...
                if (processInd >= rBufferLen) processInd = processInd - rBufferLen;
            } else {
                /*Check MID, TBD */

                /*Check payload len*/
                payloadlen = (u8)((*pCurrent)[processInd + ATOM_PAYLOAD_LEN_INDEX]);
                char char_payloadlen[128];
                sprintf(char_payloadlen, "180 payload:%d", payloadlen);
                if (payloadlen == 0xFF) {
                    processInd += 2;
                    len -= 2;
                    continue;
                    payloadlen = *((u16*)((*pCurrent) + processInd + ATOM_PAYLOAD_LEN_INDEX + 1));
                    sprintf(char_payloadlen, "184 payload:%d", payloadlen);
                ...
                if ((*punProcessedLen) >= (u32)packetLen) {
                    ...
                    char char_o[128];
                    sprintf(char_o, "238 pun:%d, pack:%d", *punProcessedLen, packetLen);
...
int Atom_DecodeData(u8* pCur, u16 payloadLenth) {
    ...
    switch (classID & 0x0F) {
        case CLASS_ID_HOSTCONTROL_CMD:
            ...
            if (msgID == (CMD_ID_SABER_DATA_PACKET | 0x80)) {
                if (errorCode != RETURN_CODE_OK) {
                    printf("ERROR: CMD_ID_GET_DATA_PAC");
                } else {
                    // Parsing packets\A3\ACpData is our saber data packets point
                    /*-----------User Code Begin----------------*/
...
int AtomCmd_Processer(u8* pHeader, u8** ppCustomer, u8* ringBuffer, u32 ringBufferLen, u32* pBytes) {
    ...
    if ((*ppCustomer) > pBufferEnd) (*ppCustomer) = pBufferStart + ((*ppCustomer) - pBufferEnd);
