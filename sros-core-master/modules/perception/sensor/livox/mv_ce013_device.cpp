...
// CODE
    ...
    LivoxMidXXModule::LivoxMidXXModule()
            : sros::core::Module("Livox_midxx_Driver"), is_update_frame_(true), is_enable_publisher_(false) {
    ...
    void LivoxMidXXModule::initLivoxMsgHead() {
        livox_msg_ptr_ = std::make_shared<sros::core::LivoxPointsMsg>();
        livox_msg_ptr_->topic_ = "LIVOX_MID70_MSG";
        livox_msg_ptr_->header = "livox_Mid70";
        livox_msg_ptr_->lidar_id = 0;
    }
    ...
    void LivoxMidXXModule::updateFrame() {
        ...
        while (is_update_frame_) {
            ...
            // send msg for record srosbag
            if (livox_msg_ptr_ == nullptr) {
                this->initLivoxMsgHead();
            }
            for (const auto &p: frame.points) {
                sros::core::LivoxPointsMsg::CustomPoint customPoint;
                customPoint.x = p.x;
                customPoint.y = p.y;
                customPoint.z = p.z;
                customPoint.reflector = 0;
                customPoint.tag = 0;
                customPoint.line = 0;
                customPoint.offset_time = 0;
                livox_msg_ptr_->points.push_back(customPoint);
            }
            if (livox_msg_ptr_->points.size() >= ACCUMULATE_MID70_FRAMES_COUNT * 96) {
                livox_msg_ptr_->point_num = livox_msg_ptr_->points.size();
                livox_msg_ptr_->time_ = sros::core::util::get_time_in_us();
                auto tmpMsgPtr = std::make_shared<sros::core::LivoxPointsMsg>();
                *tmpMsgPtr = *livox_msg_ptr_;
                sendMsg(tmpMsgPtr);
                LOG(INFO) << "succeed to send msg on topic: LIVOX_MID70_MSG, points size is: "
                          << livox_msg_ptr_->points.size();
                livox_msg_ptr_->points.clear();
            }
...
//                msg->cloud.emplace_back(p.x, p.y, p.z);
                ...
                sendMsg(msg);
//            LOG(INFO) << "@001 livox  "<< frame_index <<" frame time:" << msg->seq <<"  msg_time: "<< msg->time_
//                      << " frame.size:" << msg->cloud.size();
