...
#ifndef SROS_TF_FIX_ARRAY_HPP
#define SROS_TF_FIX_ARRAY_HPP
...
//    typedef boost::shared_lock<WR_Mutex>   readLock;
    ...
    */
    FixArray(int size_) : real_size(0), size(size_), index(0), array_(nullptr) {
        array_ = new numtype[size];
    }

    FixArray() : size(10), array_(nullptr),index(0),real_size(0) {
        array_ = new numtype[size];
    }
...
//        readLock lock(rw_mutex);
        int new_index = index + size - 1;
        int array_index = (new_index - index_) % size;
        if (array_index >= 0) {
            return array_[array_index];
        }
        LOG(INFO) << "array index is wrong!" << array_index;
        return array_[0];
    }
