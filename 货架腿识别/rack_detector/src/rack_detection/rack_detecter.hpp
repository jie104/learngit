...
class RackDetecter {
 ...
 public:
    ...
    //获取识别货架(类型以及对应的位姿)
    void detectRack(std::shared_ptr<LaserScan> origin_scan_ptr, std::vector<RackInfo> &rack_infos,
                    std::vector<Eigen::Vector3d> &center_pose) {
        std::shared_ptr<LaserScan> scan_ptr(new LaserScan);
        *scan_ptr = *origin_scan_ptr;
        filterScanTrailPoint(scan_ptr,5.0,0.02);
        racks_.clear();
        ...
        LOG(INFO) << "rack size:" << racks_.size();
        for (const auto &rack : racks_) {
            LOG(INFO) << rack.type.leg_d << ", " << rack.center.x() << ", " << rack.center.y() << ", " << rack.direction
                      << "," << rack.type.leg_groups.back().length << "," << rack.type.leg_groups.back().width;
